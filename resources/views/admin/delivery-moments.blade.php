    ...
    <div id="admin-content-wrapper" class="flex-1 flex flex-col min-w-0 transition-all duration-300 lg:pl-64">
        ...
        <main class="flex-1 max-w-[1720px] w-full mx-auto p-4 lg:p-8 space-y-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div class="lg:col-span-4 glass-island p-6 md:p-8 rounded-3xl sticky top-24">
                    <div class="mb-6">
                        ...
                        <p class="text-xs text-slate-400 mt-1">Foto yang diunggah akan otomatis muncul di slider "Delivery Moments" beranda.</p>
                    </div>
                    ...
                    <form method="POST" action="{{ route('admin.crm.deliverymoments.store') }}" enctype="multipart/form-data" class="space-y-4 text-xs">
                        ...
                        <div>
                            <label class="block font-bold uppercase tracking-wider text-slate-300 mb-2">Pilih File Foto</label>
                            <div class="border-2 border-dashed border-slate-700 hover:border-cyan-500/60 rounded-2xl p-5 text-center transition-colors bg-slate-900/60 cursor-pointer relative">
                                ...
                                <div id="upload-placeholder">
                                    ...
                                    <p class="text-white font-semibold">Klik atau seret foto ke sini</p>
                                    <p class="text-[10px] text-slate-500 mt-1">Format: JPG, JPEG, PNG, WEBP (Maks 10MB)</p>
                                </div>

                                <div id="upload-preview-wrapper" class="hidden">
                                    <img id="upload-preview" src="#" alt="Preview" class="max-h-48 mx-auto rounded-xl object-contain border border-slate-700">
                                    <p class="text-[10px] text-cyan-400 mt-2 font-mono" id="upload-filename"></p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="display_order" value="0" min="0" placeholder="0" 
                                   class="w-full px-4 py-3 bg-slate-900 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:border-cyan-400">
                            <p class="text-[10px] text-slate-500 mt-1">Angka lebih kecil akan tampil lebih depan pada slider.</p>
                        </div>
                ...
                <div class="lg:col-span-8 glass-island p-6 md:p-8 rounded-3xl">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                        <div>
                            ...
                            <p class="text-xs text-slate-400">Total: {{ $moments->total() }} foto tercatat di database.</p>
                        </div>
                    </div>

                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
                        @forelse($moments as $moment)
                        <div class="rounded-2xl bg-slate-900/90 border border-slate-800 overflow-hidden flex flex-col justify-between hover:border-slate-700 transition-colors group">
                            
                            <div class="relative w-full aspect-[4/5] bg-slate-950 overflow-hidden">
                                <img src="{{ $moment->image_url }}" alt="{{ $moment->caption }}" class="w-full h-full object-cover object-center group-hover:scale-105 transition-transform duration-500">
                                
                                <div class="absolute top-2.5 left-2.5 px-2 py-0.5 rounded-md bg-slate-950/80 backdrop-blur-md border border-slate-700 text-[10px] font-mono text-cyan-300">
                                    Urutan: {{ $moment->display_order }}
                                </div>
                            </div>

                            <div class="p-3.5 flex flex-col justify-between flex-1 border-t border-slate-800">
                                <div class="mb-3">
                                    <p class="text-xs font-bold text-white line-clamp-2">{{ $moment->caption ?: 'Momen Serah Terima Unit' }}</p>
                                    <p class="text-[10px] text-slate-500 mt-0.5">{{ $moment->created_at ? $moment->created_at->format('d M Y') : '-' }}</p>
                                </div>
                                ...
                                <form method="POST" action="{{ route('admin.crm.deliverymoments.destroy', $moment) }}" onsubmit="return confirm('Apakah Anda yakin ingin menghapus foto ini dari galeri beranda?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="w-full py-2 px-3 rounded-xl bg-rose-950/50 hover:bg-rose-900 border border-rose-800/80 text-rose-300 hover:text-white text-[11px] font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 transition-colors cursor-pointer">
                        ...
                        <div class="col-span-full py-16 text-center text-slate-500 text-xs">
                            ...
                            <p class="font-bold text-slate-400">Belum ada foto yang diunggah.</p>
                            <p class="mt-1 text-[11px]">Gunakan formulir di sebelah kiri untuk mengunggah foto serah terima pertama Anda.</p>
                        </div>
                        @endforelse
                    </div>
