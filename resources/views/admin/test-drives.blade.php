...
<head>
    ...
    <title>Manajemen Test Drive Armada BSD | Promo Geely CRM</title>
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="flex items-center gap-2">
                        <span class="font-extrabold tracking-tight text-white uppercase text-sm md:text-base leading-none font-geely">Armada & Test Drive</span>
                        <span class="px-2 py-0.5 rounded-full text-[9px] font-bold tracking-widest uppercase bg-sky-950 text-cyan-300 border border-sky-600/40">BSD Fleet</span>
                    </div>
                    <span class="text-[10px] text-slate-400 tracking-wider">Alokasi & Penjadwalan Unit Uji Jalan Showroom</span>
                </div>
            </div>
                ...
                <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-sky-950/80 border border-sky-500/30 text-cyan-300 text-[10px] font-bold tracking-widest uppercase mb-3">
                            ...
                            <span>Armada Showroom BSD Standby</span>
                        </div>
                        <h1 class="text-2xl md:text-3xl font-extrabold uppercase tracking-tight text-white font-geely">
                            Jadwal & Armada <span class="text-gradient-cyan">Test Drive</span>
                        </h1>
                        <p class="text-slate-400 text-xs md:text-sm mt-1 max-w-xl">
                            Kelola jadwal uji jalan konsumen untuk unit Geely EX5, EX2, dan Starray EM-i. Pastikan data pemesan dan nomor WhatsApp terverifikasi sebelum penugasan.
                        </p>
                    </div>

                    <div class="flex flex-wrap gap-3">
                        <div class="p-3.5 rounded-2xl bg-slate-900/90 border border-slate-800 text-center min-w-[130px]">
                            <span class="text-[10px] uppercase font-bold text-slate-400 block">Unit EX5 Standby</span>
                            <span class="text-sm font-mono font-bold text-cyan-300">Showroom BSD</span>
                        </div>
                        <div class="p-3.5 rounded-2xl bg-slate-900/90 border border-slate-800 text-center min-w-[130px]">
                            <span class="text-[10px] uppercase font-bold text-slate-400 block">Unit EX2 Standby</span>
                            <span class="text-sm font-mono font-bold text-sky-400">Showroom BSD</span>
                        </div>
                        <div class="p-3.5 rounded-2xl bg-slate-900/90 border border-slate-800 text-center min-w-[130px]">
                            <span class="text-[10px] uppercase font-bold text-slate-400 block">Starray EM-i</span>
                            <span class="text-sm font-mono font-bold text-emerald-400">Showroom BSD</span>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Tabel Jadwal Test Drive -->
            <div class="glass-island rounded-3xl p-6 md:p-8">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                    <div>
                        <h3 class="text-base font-bold uppercase tracking-tight text-white font-geely">Daftar Reservasi Test Drive</h3>
                        <p class="text-xs text-slate-400">Pembaruan status konfirmasi jadwal, armada uji jalan, dan pelat nomor</p>
                    </div>
                    <span class="px-3 py-1 rounded-full text-xs font-mono font-bold bg-sky-950 text-cyan-300 border border-sky-800">
                        Total: {{ $testDrives->total() }} Reservasi
                    </span>
                </div>
                    ...
                    <table class="w-full text-left border-collapse text-xs">
                        <thead>
                            <tr class="border-b border-slate-800 text-slate-400 text-[10px] uppercase tracking-wider">
                                ...
                                <th class="py-3 px-4">Lokasi / Venue</th>
                                <th class="py-3 px-4">Sales PIC</th>
                                <th class="py-3 px-4">Status</th>
                                <th class="py-3 px-4 text-right">Aksi & WhatsApp</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-800/60">
                            ...
                            <tr class="hover:bg-slate-800/40 transition-colors">
                                <td class="py-3.5 px-4 font-bold text-white">
                                    <p class="text-white font-bold">{{ $td->lead->name ?? 'Konsumen Web' }}</p>
                                    <p class="text-[10px] text-slate-500 font-mono">{{ $td->lead->phone ?? '-' }}</p>
                                </td>
                                <td class="py-3.5 px-4">
                                    <span class="font-semibold text-cyan-300">{{ $td->car_model }}</span>
                                    <span class="block text-[10px] font-mono text-slate-400">{{ $td->test_plate ?? 'Unit Showroom BSD' }}</span>
                                </td>
                                <td class="py-3.5 px-4">
                                    <p class="text-white font-semibold">{{ $td->scheduled_at ? $td->scheduled_at->format('d M Y') : '-' }}</p>
                                    <p class="text-[10px] text-cyan-400 font-mono">{{ $td->scheduled_at ? $td->scheduled_at->format('H:i') . ' WIB' : '-' }}</p>
                                </td>
                                ...
                                <td class="py-3.5 px-4">
                                    <form method="POST" action="{{ route('admin.crm.testdrives.updateStatus', $td) }}" class="inline-block">
                                        @csrf
                                        @method('PATCH')
                                        <select name="status" onchange="this.form.submit()" 
                                                ...
                                                {{ $td->status === 'Confirmed' ? 'text-emerald-300 border-emerald-800' : '' }}
                                                {{ $td->status === 'Pending' ? 'text-amber-300 border-amber-800' : '' }}
                                                {{ $td->status === 'Completed' ? 'text-sky-300 border-sky-800' : '' }}
                                                {{ $td->status === 'Cancelled' ? 'text-rose-400 border-rose-800' : '' }}">
                                ...
                                <td class="py-3.5 px-4 text-right">
                                    @if($td->lead)
                                        <a href="https://wa.me/{{ $td->lead->whatsapp_number }}?text={{ rawurlencode('Halo ' . $td->lead->name . ', saya ' . $td->sales_pic . ' dari Showroom Geely BSD. Kami mengonfirmasi jadwal test drive mobil ' . $td->car_model . ' Anda pada ' . ($td->scheduled_at ? $td->scheduled_at->format('d M Y jam H:i') : '') . ' WIB di ' . $td->venue . '. Apakah jadwal tersebut sudah sesuai?') }}" 
                                           target="_blank"
                            ...
                            <tr>
                                <td colspan="7" class="py-10 text-center text-slate-500">
                                    Belum ada antrean reservasi test drive.
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
